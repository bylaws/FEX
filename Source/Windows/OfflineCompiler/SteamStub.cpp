...
TryDecryptSteamStubResult TryDecryptSteamStubIfPresent(uint64_t BaseAddress) {
  ...
  if (BCryptImportKey(*AESECB, nullptr, BCRYPT_KEY_DATA_BLOB, &Key, nullptr, 0, reinterpret_cast<uint8_t *>(&KeyDataBlob),
                      sizeof(KeyDataBlob), 0)) {
    return TryDecryptSteamStubResult::DecryptFailure;
  }
  ...
  bool Success = !BCryptDecrypt(Key, HeaderCopy.TextAESIV.data(), HeaderCopy.TextAESIV.size(), nullptr, nullptr, DecryptedIV.size(),
                                DecryptedIV.data(), DecryptedIV.size(), &DecyptedSize, 0) && DecyptedSize == DecryptedIV.size();
  ...
  if (BCryptImportKey(*AESCBC, nullptr, BCRYPT_KEY_DATA_BLOB, &Key, nullptr, 0, reinterpret_cast<uint8_t *>(&KeyDataBlob),
                      sizeof(KeyDataBlob), 0)) {
    return TryDecryptSteamStubResult::DecryptFailure;
  }

  Success = !BCryptDecrypt(Key, TextBegin, HeaderCopy.TextSize, nullptr, DecryptedIV.data(), DecryptedIV.size(),
                           TextBegin, HeaderCopy.TextSize, &DecyptedSize, 0) && DecyptedSize == HeaderCopy.TextSize;
  BCryptDestroyKey(Key);
  if (!Success) {
    return TryDecryptSteamStubResult::DecryptFailure;
  }
